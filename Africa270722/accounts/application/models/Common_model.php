...
class Common_model extends CI_Model {
   ...
   function check_follower($id)
   {
    ...
    public function get_table_records($table_name, array $columns, $primary_key, $id = 0, $filter_condition = null) {
        try
        {
            if ($id && !is_null_or_empty_string($filter_condition))
            {
                $table_records =
                    $this
                        ->db
                        ...
                        ->get_where($table_name, array($primary_key => $id));

                if($table_records->num_rows())
                {
                    $result_set['count'] = $table_records->num_rows();
                    $result_set['data'] = $table_records->row();
                    return $result_set;
                }
                return 0;
            }
            ...
            else if(!$id && !is_null_or_empty_string($filter_condition))
            {
                $table_records =
                    $this
                        ->db
                ...
                {
                    $result_set['count'] = $table_records->num_rows();
                    $result_set['data'] = $table_records->result();
                    return $result_set;
                }
                return 0;
            }
        ...
        {
            log_message('error', "Model: ".$this->router->fetch_class(). " Model Method: ".$this->router->fetch_method()." Exception: ".$e->getMessage());
        }
    }

    public function get_table_records_crud($table_name, array $columns, $primary_key = null, $id = 0, $filter_condition = null, $order_by_column = null, $order_by_value = null, $limit = 0) {
        try
        {
            if ($id && !is_null_or_empty_string($filter_condition) && !is_null_or_empty_string($primary_key))
            {
                $table_records =
                    $this
                        ->db
                ...
                return $table_records->num_rows() > 0 ?  $table_records->row() : 0;
            } else if ($id && is_null_or_empty_string($filter_condition) && !is_null_or_empty_string($primary_key)) {
                $table_records =
                    $this
                        ->db
                        ->select($columns)
                ...
                return $table_records->num_rows() > 0 ?  $table_records->row() : 0;
            } else if(!$id && !is_null_or_empty_string($filter_condition) && is_null_or_empty_string($primary_key) && !is_null_or_empty_string($order_by_column) && !is_null_or_empty_string($order_by_value) && $limit > 0) {
                $table_records =
                    $this
                        ->db
                        ->select($columns)
                  		->order_by($order_by_column, $order_by_value)
                        ->where($filter_condition)
                  		->limit($limit)	
                        ->get($table_name);
              			#log_message('error', $this->db->last_query());
                        #echo "4".$this->db->last_query();exit;
                return $table_records->num_rows() > 0 ?  $table_records->result() : 0;
            } else if(!$id && !is_null_or_empty_string($filter_condition) && is_null_or_empty_string($primary_key) && !is_null_or_empty_string($order_by_column) && !is_null_or_empty_string($order_by_value)) {
                $table_records =
                    $this
                        ->db
                        ->select($columns)
                ...
                $table_records =
                    $this
                        ...
                        ->get($table_name);
              			#log_message('error', $this->db->last_query());
                        #echo "5".$this->db->last_query();exit;
                return $table_records->num_rows() > 0 ?  $table_records->result() : 0;
            } else if(!$id && is_null_or_empty_string($filter_condition) && is_null_or_empty_string($primary_key) && !is_null_or_empty_string($order_by_column) && is_null_or_empty_string($order_by_value)) {
                $table_records =
                    $this
                        ...
                        ->get($table_name);
              			#log_message('error', $this->db->last_query());
                        #echo "6".$this->db->last_query();exit;
                return $table_records->num_rows() > 0 ?  $table_records->result() : 0;
            } else if(!$id && !is_null_or_empty_string($filter_condition) && is_null_or_empty_string($primary_key) && is_null_or_empty_string($order_by_column) && is_null_or_empty_string($order_by_value)) {
                $table_records =
                    $this
                        ...
                        ->get($table_name);
                        #log_message('error', $this->db->last_query());
                        #echo "7".$this->db->last_query();exit;
                return $table_records->num_rows() > 0 ?  $table_records->result() : 0;
            } else {
                $table_records =
                    $this
                        ...
                        ->get($table_name);
              			#log_message('error', $this->db->last_query());
                        #echo "8".$this->db->last_query();exit;
                return $table_records->num_rows() > 0 ?  $table_records->result() : 0;
            }
        }
        catch (Exception $e)
    ...
    public function delete_table_records($table_name, $primary_key, $id = 0, $filter_condition = "") {
        try
        {
            if (!is_null_or_empty_string($table_name))
            {
                if ($id && !is_null_or_empty_string($primary_key) && !is_null_or_empty_string($filter_condition))
                {
                    $is_deleted =
                        $this
                            ->db
                            ->where($filter_condition)
                            ->delete($table_name,  array($primary_key => $id));
                    return $is_deleted;
                }
                if (!$id && is_null_or_empty_string($primary_key) && !is_null_or_empty_string($filter_condition))
                {
                ...
                // When id is passed without any extra filter condition
                if ($id && !is_null_or_empty_string($primary_key) && is_null_or_empty_string($filter_condition))
                {
                    $is_deleted =
                        $this
    ...
    public function get_table_records_count($table_name, $filter_condition = null) {
        if (!is_null_or_empty_string($filter_condition)) {
            $this
                ->db
                ->select('*')
                ->from($table_name)
		...
		$table_records = $this->db->get();
        return $table_records->num_rows();
    }

    public function insert_record($table_name, $table_records) {
        try {
            $this->db->trans_begin();

            $result = $this
                ->db
                ->insert($table_name, $table_records);
    ...
    public function insert_batch_record($table_name, $table_records) {
        try
        {
            $this->db->trans_begin();
            ...
            $result = $this
                ->db
                ->insert_batch($table_name, $table_records);
    ...
    public function update_record($table_name, $table_records, $primary_key, $primary_key_id, $update_condition = null) {
        try
        {
            $this->db->trans_begin();

            if (!empty($primary_key) && !empty($primary_key_id)) {
                $result = $this
                    ->db
                    ->where($primary_key, $primary_key_id);
            }

            if (!empty($update_condition) && !empty($primary_key) && !empty($primary_key_id)) {
                $result = $this
                    ->db
                    ->where($primary_key, $primary_key_id)
            ...
            $result = $this
                ->db
                ->update($table_name, $table_records);
            #echo $this->db->last_query();exit;
            #log_message('error', $this->db->last_query());
            if($result) {
                $this->db->trans_complete();
    ...
    public function get_distinct_record_from_table_column_name($table_name, $column_name, $value) {
        try {
            $data = $this
                ->db
                ->select($column_name)
                ->get_where($table_name, array(
                        $column_name => $value
                    )
                );
            #echo $this->db->last_query();exit;
